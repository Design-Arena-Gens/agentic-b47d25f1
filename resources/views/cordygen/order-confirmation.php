...
<section class="bg-white py-16">
    <div class="max-w-4xl mx-auto px-4">
        <div class="border border-orange-200 bg-orange-50 rounded-3xl p-8 text-center">
            <div class="w-14 h-14 rounded-full bg-orange-500/20 text-orange-700 flex items-center justify-center mx-auto mb-4">
                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M5 13l4 4L19 7"></path>
                </svg>
            </div>
            <h1 class="text-3xl font-bold text-orange-900">Thank you for choosing Cordygen!</h1>
            <p class="mt-3 text-sm text-orange-800">Order <strong><?= htmlspecialchars($order['order_number']) ?></strong> is confirmed. Expect a confirmation email with tracking shortly.</p>
        </div>
        ...
        <div class="mt-10 grid md:grid-cols-2 gap-8">
            <div class="border border-slate-200 rounded-2xl p-6">
                ...
                <dl class="mt-4 text-sm text-slate-600 space-y-2">
                    <div class="flex justify-between"><dt>Status</dt><dd class="font-semibold text-orange-600"><?= htmlspecialchars($order['status']) ?></dd></div>
                ...
                <?php if ($payment): ?>
                    <div class="mt-4 rounded-xl bg-slate-100 p-4 text-xs text-slate-600">
                        ...
                        <p class="mt-2">Complete payment via Razorpay. Ping support if you faced any issue.</p>
                    </div>
                <?php endif; ?>
            </div>
            <div class="border border-slate-200 rounded-2xl p-6">
        ...
        <div class="mt-12 border border-slate-200 rounded-2xl p-6">
            ...
            <div class="mt-4 space-y-4">
                <?php foreach ($items as $item): ?>
                    <div class="flex items-center justify-between border border-slate-100 rounded-xl p-4 bg-white">
                        <div>
        ...
        <div class="mt-10 flex flex-col sm:flex-row gap-4">
            <a href="/cordygen/products" class="brand-button inline-flex items-center justify-center px-5 py-3 rounded-full font-semibold">Continue Shopping</a>
            <a href="/cordygen/order-track?order_number=<?= urlencode($order['order_number']) ?>" class="inline-flex items-center justify-center px-5 py-3 border border-orange-500 text-orange-600 rounded-full font-semibold hover:bg-orange-50 transition">Track Order</a>
        </div>
    </div>
</section>
<?php
