...
<section class="bg-white py-16">
    <div class="max-w-4xl mx-auto px-4">
        ...
        <form class="mt-6 flex flex-col sm:flex-row gap-4 items-start" method="get" action="/shnikh/order-track">
        ...
        <?php if ($order): ?>
            <div class="mt-10 border border-slate-200 rounded-2xl p-6 bg-slate-50">
                ...
                <p class="mt-2 text-sm text-slate-600">Status: <span class="font-semibold text-emerald-700"><?= htmlspecialchars($order['status']) ?></span></p>
        ...
        <?php elseif (!empty($_GET['order_number'])): ?>
            <div class="mt-10 border border-red-200 bg-red-50 text-sm text-red-600 rounded-2xl p-5">
                We could not find an order with that number. Double check and try again.
            </div>
        <?php endif; ?>
    </div>
</section>
